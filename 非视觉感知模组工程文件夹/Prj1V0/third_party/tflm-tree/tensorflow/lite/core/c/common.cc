...
int32_t MultiplyByQuantizedMultiplier(int64_t x, int32_t quantized_multiplier,
  ...
  const int64_t total_shift = 31 - shift;
  const int64_t round = static_cast<int64_t>(1) << (total_shift - 1);
  int64_t result = (x * quantized_multiplier) + round;
  result = result >> total_shift;
  return static_cast<int32_t>(result);
}
#else
...
int32_t MultiplyByQuantizedMultiplier(int32_t x, int32_t quantized_multiplier,
                                      int shift) {
  const int64_t total_shift = 31 - shift;
  int64_t result = x * static_cast<int64_t>(quantized_multiplier);
  int64_t doubled = result << 1;
  const int64_t round = static_cast<int64_t>(1) << (total_shift - 1);
  int64_t rounded = (doubled + round) >> (total_shift + 1);
...
int32_t MultiplyByQuantizedMultiplier(int64_t x, int32_t quantized_multiplier,
                                      int shift) {
  const int64_t total_shift = 31 - shift;
  int64_t result = x * quantized_multiplier;
  int64_t doubled = result << 1;
  const int64_t round = static_cast<int64_t>(1) << (total_shift - 1);
  int64_t rounded = (doubled + round) >> (total_shift + 1);
  return static_cast<int32_t>(rounded);
