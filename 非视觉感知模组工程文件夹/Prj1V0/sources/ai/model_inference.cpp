...
static uint8_t g_tensor_arena[MODEL_INTERPRETER_BUFFER_SIZE];
...
model_inference_handle_t g_model_handle = {
    { MODEL_TYPE_FALL_DETECTION, 0xFF, 0, 0 },
    { 0, 0.0f, 0, 0 },
    { 128, 64, 100, 1, 1 },
    nullptr,
    nullptr
...
/* 统计变量 */
static uint64_t g_total_inference_time_ticks = 0;
...
int model_load_from_memory(const uint8_t *model_data, uint32_t size)
{
    ...
    /* 创建解释器 - 使用 MicroAllocator 构造函数 */
    tflite::MicroAllocator *allocator = tflite::MicroAllocator::Create(
        g_tensor_arena, MODEL_INTERPRETER_BUFFER_SIZE);
    if (allocator == nullptr) {
        return -3;
    }

    g_interpreter = new tflite::MicroInterpreter(
        g_model, g_resolver, allocator, nullptr, nullptr);
    ...
    if (status != kTfLiteOk) {
        tflite::GetMicroErrorReporter()->Report("AllocateTensors failed!");
        return -5;
    }
 ...
 * @brief   运行推理
 */
int model_run_inference(const float *input_data, uint16_t input_len,
                       model_result_t *result)
{
    ...
    /* 记录开始时间 */
    start_time = tflite::GetCurrentTimeTicks();
    ...
    if (status != kTfLiteOk) {
        tflite::GetMicroErrorReporter()->Report("Inference failed!");
        result->is_valid = 0;
    ...
    /* 记录结束时间 */
    end_time = tflite::GetCurrentTimeTicks();
    ...
    /* 计算推理耗时 (ticks -> us，假设 1 tick = 1 us，或按 tflite::ticks_per_second 换算) */
    uint32_t inference_ticks = (end_time >= start_time)
                                ? (end_time - start_time)
                                : 0;
    ...
    result->inference_time_us = inference_ticks;
    result->is_valid = 1;

    /* 更新统计 */
    g_model_handle.last_result = *result;
    g_model_handle.current_model.last_inference_time = inference_ticks;
    g_model_handle.current_model.total_inferences++;
    g_total_inference_time_ticks += inference_ticks;
...
uint32_t model_get_avg_inference_time(void)
{
    ...
    return (uint32_t)(g_total_inference_time_ticks / g_total_inference_count);
}
