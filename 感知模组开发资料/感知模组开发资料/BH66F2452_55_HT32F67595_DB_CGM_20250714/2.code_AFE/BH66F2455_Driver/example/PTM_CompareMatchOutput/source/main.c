...
void main()
{
    ...
    if (_to == 0 || _pdf == 0)
    {
        ...
        // config PTM to compare match output mode
        PTM_Cfg_CompareMatchOutputMode_t cfg;
        cfg.clockSource = PTM_CLOCK_FSYS_4;
        cfg.outputMode  = PTM_COMPAREMATCH_INIT_L_OUTPUT_TOGGLE;
        cfg.countClear  = PTM_COUNT_CLEAR_A_MATCH;
        cfg.pinInvert   = false;
        cfg.count       = 500;
        PTM_Cfg_CompareMatchOutput(&cfg);
